...
function dataDir() { return process.env.CADENCE_DATA_DIR ? path.resolve(process.env.CADENCE_DATA_DIR) : path.join(__dirname, '..', 'data'); }
...
async function runRound(opts = {}) {
  ...
  const ctx = makeCtx(opts.dir || process.env.CADENCE_DIR || '.');
...
if (require.main === module) {
  ...
  const dir = OPT('dir', process.env.CADENCE_DIR || '.');
  ...
  (async () => {
    for (let round = 1; round <= rounds; round++) {
      ...
      console.log(`\n[cadence] round ${round === Infinity ? '' : round} · dir=${path.resolve(dir)} · ${r.ms}ms · minted ${r.minted}`);
      console.log('health:');
